...
def _latest_to_reading(latest: dict, equipment_id: str) -> SensorReading:
    return SensorReading(
        ...
        seal_condition_pct=float(latest["seal_condition_pct"])
        if latest.get("seal_condition_pct")
        else None,
...
def register(app) -> None:
    ...
    def update_overview(n_intervals: int):
        ...
        for eq_id, eq in EQUIPMENT_CONFIG.items():
            ...
            alert_color = (
                "#da3633" if active_count > 5 else "#e8a020" if active_count > 0 else "#2ea44f"
            )
                                ...
                                [
                                    html.Div(
                                        [
                                            html.Div(
                                                "Health Index",
                                                style={"fontSize": ".65rem", "color": MUTED},
                                            ),
                                            html.Div(
                                                f"{hi:.1f}%",
                                                style={
                                    ...
                                    html.Div(
                                        [
                                            html.Div(
                                                "Modo", style={"fontSize": ".65rem", "color": MUTED}
                                            ),
                                            html.Div(
                                    ...
                                    html.Div(
                                        [
                                            ...
                                            html.Div(
                                                f"{latest['throughput_tph']:.0f} t/h",
                                                style={
                                                    "fontSize": ".82rem",
                                                    "fontWeight": "600",
                                                    "color": "#c9d1d9",
        ...
        total_critical = (
            len(store.get_alerts(days=1).query("severity == 'critical'"))
            if not store.get_alerts(days=1).empty
            else 0
        )
        ...
        kpi_banner = dbc.Row(
            [
                dbc.Col(
                    kpi_card(
                        "Salud de Flota", f"{fleet_hi:.1f}%", fleet_color, border_color=fleet_color
                    ),
                    xs=6,
                    md=3,
                ),
                dbc.Col(
                    kpi_card("Equipos Monitoreados", str(len(EQUIPMENT_CONFIG)), "#58a6ff"),
                    xs=6,
                    md=3,
                ),
                dbc.Col(
                    kpi_card(
                        "Alertas Activas",
                        str(total_alerts),
                        "#e8a020" if total_alerts > 0 else "#2ea44f",
                    ),
                    xs=6,
                    md=3,
                ),
                dbc.Col(
                    kpi_card(
                        ...
                        str(total_critical),
                        "#da3633" if total_critical > 0 else "#2ea44f",
                    ),
                    xs=6,
                    md=3,
                ),
        ...
        if alerts_df.empty:
            alerts_table = html.Div(
                "Sin alertas recientes.", style={"color": MUTED, "padding": "12px"}
            )
        else:
            rows = []
                    ...
                    html.Tr(
                        [
                            html.Td(
                                ...
                                style={"fontSize": ".72rem", "color": MUTED},
                            ),
                            html.Td(
                                html.Span(
                                    row["equipment_id"],
                                    style={"color": "#58a6ff", "fontSize": ".78rem"},
                                )
                            ),
                            html.Td(
                                html.Span(
                                    SEVERITY_LABELS_ES.get(row["severity"], row["severity"]),
                                    style={
                                        "color": sev_color,
                                        "fontSize": ".72rem",
                                        "fontWeight": "700",
                ...
                [
                    html.Thead(
                        html.Tr(
                            [
                                html.Th(h)
                                for h in ["Hora", "Equipo", "Severidad", "Variable", "Mensaje"]
                            ],
                            style={
                                "color": MUTED,
                                "fontSize": ".65rem",
